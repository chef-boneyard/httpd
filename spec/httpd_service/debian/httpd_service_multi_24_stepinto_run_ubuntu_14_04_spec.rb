...
describe 'httpd_service::multi 2.4 on ubuntu-14.04' do
  ...
  context 'when stepping into the httpd_service[instance-1] resource' do
    ...
    it 'deletes directory[instance-1 create /var/run/apache2]' do
      expect(ubuntu_14_04_multi_stepinto_run).to_not delete_directory('instance-1 create /var/run/apache2').with(
        :path => '/var/run/apache2',
        :recursive => true
        )
    end
  ...
  context 'when stepping into the httpd_service[instance-2] resource' do
    ...
    it 'deletes directory[instance-2 create /var/run/apache2]' do
      expect(ubuntu_14_04_multi_stepinto_run).to_not delete_directory('instance-2 create /var/run/apache2').with(
        :path => '/var/run/apache2',
        :recursive => true
        )
    end
