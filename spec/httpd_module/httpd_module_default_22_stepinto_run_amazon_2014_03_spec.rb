...
describe 'httpd_module::default on amazon-2014.04' do
  ...
  context 'when using default parameters' do
    ...
    it 'create execute[auth_basic create remove_package_config]' do
      expect(httpd_module_default_22_stepinto_run_amazon_2014_03).to_not run_execute('auth_basic create remove_package_config').with(
        :user => 'root',
    ...
    it 'create directory[auth_basic create /etc/httpd/conf.modules.d]' do
      expect(httpd_module_default_22_stepinto_run_amazon_2014_03).to create_directory('auth_basic create /etc/httpd/conf.modules.d').with(
        :owner => 'root',
        :group => 'root',
        :recursive => true
        )
    ...
    it 'create template[auth_basic create /etc/httpd/conf.modules.d/auth_basic.load]' do
      expect(httpd_module_default_22_stepinto_run_amazon_2014_03).to create_template('auth_basic create /etc/httpd/conf.modules.d/auth_basic.load').with(
        :owner => 'root',
        :group => 'root',
    ...
    it 'create execute[auth_kerb create remove_package_config]' do
      expect(httpd_module_default_22_stepinto_run_amazon_2014_03).to_not run_execute('auth_kerb create remove_package_config').with(
        :user => 'root',
        :command => 'rm -rf /etc/httpd/conf.modules.d'
        )
    end
    ...
    it 'create directory[auth_kerb create /etc/httpd/conf.modules.d]' do
      expect(httpd_module_default_22_stepinto_run_amazon_2014_03).to create_directory('auth_kerb create /etc/httpd/conf.modules.d').with(
        :owner => 'root',
        :group => 'root',
        :recursive => true
        )
    ...
    it 'create template[auth_kerb create /etc/httpd/conf.modules.d/auth_kerb.load]' do
      expect(httpd_module_default_22_stepinto_run_amazon_2014_03).to create_template('auth_kerb create /etc/httpd/conf.modules.d/auth_kerb.load').with(
        :owner => 'root',
        :group => 'root',
        :source => 'module_load.erb',
        :mode => '0644'
