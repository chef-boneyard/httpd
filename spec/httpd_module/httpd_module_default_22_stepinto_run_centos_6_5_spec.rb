...
describe 'httpd_module::default on centos-6.5' do
  ...
  context 'when using default parameters' do
    ...
    it 'create directory[auth_basic create /etc/httpd/conf.modules.d]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to create_directory('auth_basic create /etc/httpd/conf.modules.d').with(
        :owner => 'root',
        :group => 'root',
        :recursive => true
        )
    ...
    it 'create template[auth_basic create /etc/httpd/conf.modules.d/auth_basic.load]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to create_template('auth_basic create /etc/httpd/conf.modules.d/auth_basic.load').with(
        :owner => 'root',
        :group => 'root',
    ...
    it 'create directory[auth_kerb create /etc/httpd/conf.modules.d]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to create_directory('auth_kerb create /etc/httpd/conf.modules.d').with(
        :owner => 'root',
        :group => 'root',
        :recursive => true
        )
    ...
    it 'create template[auth_kerb create /etc/httpd/conf.modules.d/auth_kerb.load]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to create_template('auth_kerb create /etc/httpd/conf.modules.d/auth_kerb.load').with(
        :owner => 'root',
        :group => 'root',
        :source => 'module_load.erb',
        :mode => '0644'
