...
describe 'httpd_module::default on centos-6.5' do
  ...
  context 'when using default parameters' do
    ...
    it 'deletes file[auth_basic create /etc/httpd/conf.d/README]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to_not delete_file('auth_basic create /etc/httpd/conf.d/README').with(
        :path => '/etc/httpd/conf.d/README'
        )
    end

    it 'deletes file[auth_basic create httpd]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to_not delete_file('auth_basic create /etc/httpd/conf.d/welcome.conf').with(
        :path => '/etc/httpd/conf.d/welcome.conf'
        )
    end

    it 'deletes file[auth_basic create httpd]' do
      expect(httpd_module_default_22_stepinto_run_centos_6_5).to_not delete_file('auth_basic create /etc/httpd/conf.d/proxy_ajp.conf').with(
