...
describe 'httpd_module::default on centos-5.8' do
  ...
  context 'when using default parameters' do
    ...
    it 'create template[auth_basic create /etc/httpd/conf.d/auth_basic.load]' do
      expect(httpd_module_default_22_stepinto_run_centos_5_8).to create_template('auth_basic create /etc/httpd/conf.d/auth_basic.load').with(
        :owner => 'root',
        :group => 'root',
    ...
    it 'create template[auth_kerb create /etc/httpd/conf.d/auth_kerb.load]' do
      expect(httpd_module_default_22_stepinto_run_centos_5_8).to create_template('auth_kerb create /etc/httpd/conf.d/auth_kerb.load').with(
        :owner => 'root',
        :group => 'root',
        :source => 'module_load.erb',
        :mode => '0644'
